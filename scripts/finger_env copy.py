...
class EnvConfig:
    ...
    # Action ist delta joint angles pro Step (rad)
    ...
    # penalties
    w_close: float = 0.1          # Terminal Scließ-Strafe [0.2, 0.4, 0.5]
    w_close_dense: float = 0.01   # Dense Schließ-Strafe
    w_degen: float = 0.1          # Degenerierung-Strafe
    w_action: float = 0.05        # Energie-Strafe:[0.2, 0.4, 0.5] sum ||action||^2
...
class FingerEllipseEnv(gym.Env):
    ...
    def terminal_penalty(self) -> Tuple[float, Dict[str, Any]]:
        ...
        return float(penalty), penalty_info


    def reward_function(self, action: np.ndarray):
        # Dense-REWARD Design
        close_dist = float(np.sum((self.p - self.p0)**2))
        phase = (self.t + 1) / self.cfg.horizon

        ## 1. close reward ab dem dritten Drittel der Episode: fördert Schließung der Ellipse
        alpha_t = max(0, (phase - 0.7) / 0.3)       #    Bei 30% der Episode, anfangen mit der Ellipse-Schließung
        r_close_dense = self.cfg.w_close_dense * alpha_t * (self.prev_close_dist - close_dist)
        ...
        r_area_dense = area_now - self.prev_area
        self.prev_area = area_now

        ## dense_reward zusammen summieren
        reward = float(r_area_dense - r_action_dense + r_close_dense)
    ...
    def step(self, action: np.ndarray):
        ...
        if self.cfg.adv_noise_scale > 0.0:
            eps = self.np_random.uniform(-1.0, 1.0, size=(3,))  # Random-Wert: verhindert Overfitting
...
def run_episode(seed):
    cfg = EnvConfig(horizon=256, adv_noise_scale=0.0)   #256 Environment-Horizon(Env. Ebene = maximale Episodenlänge) = 256. Rollout-Horizon (Parameter-Update) ist Training-Ebene
